...
function [eff_th_fluid, eff_th_cool, eff_th_total, eff_el, T_r, T_sspvt, Energy, P_el, P_th] = sspvt_performance(Ta, Tcin, Tflin, GG, Vwind, mc, mfl);
    ...
    for j=1:NG(2)
        ...
        for i=1:Ntr(2)
            ...
            %efficiencies
            T_r(i)=(0.5*T(3)+0.5*T(4)-Ta(j))/G;
            eff_th_fluid(i)=mfl(j)*cpfl*(T(4)-T(3))/G/Ac;
            eff_th_cool(i)=mc(j)*cpc*(T(9)-T(8))/G/Ac;
            eff_th_total(i)=eff_th_fluid(i)+eff_th_cool(i);
            eff_el(i)=effm;
            ...
            %energy balance
            Energy(i,1)=hr_g2sky*(T(1)-Tsky(j))/G;
            Energy(i,2)=hwind*(T(1)-Ta(j))/G;
            ...
            Energy(i,8)=mfl(j)*cpfl*(T(4)-T(3))/G/Ac;
            Energy(i,9)=hc_cool*(T(7)-0.5*T(8)-0.5*T(9))/G;
            Energy(i,10)=h_cool2amb*(0.5*T(8)+0.5*T(9)-Ta(j))/G;
            Energy(i,11)=mc(j)*cpfl*(T(9)-T(8))/G/Ac;
            
            % Power output
            P_el(i)=effm*G*Ac;
            P_th(i)=mc(j)*cpfl*(T(9)-T(8)) + mfl(j)*cpfl*(T(4)-T(3));
